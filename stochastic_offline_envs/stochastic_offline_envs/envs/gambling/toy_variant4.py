...
class ToyStochasticGame(gym.Env):
    ...
    def __init__(self):
        ...
        self.state = 0
        self.reward_list = [5, 5, 0, 15, 1, 2, 8, 10]
    ...
    def step(self, action):
        ...
        if self.state == 0:
            if action == 0:
                if adv_action == 0:  
                    self.state = np.random.choice([1, 2], p=[0.9, 0.1]) 
                elif adv_action == 1:
                    self.state = np.random.choice([1, 2], p=[0.2, 0.8])
            elif action == 1:
                if adv_action == 0:
                    self.state = np.random.choice([3, 4], p=[0.5, 0.5])
                elif adv_action == 1:
                    self.state = np.random.choice([3, 4], p=[0.8, 0.2])
            elif action == 2:
                if adv_action == 0:
                    self.state = np.random.choice([3, 4], p=[0.2, 0.8])
                elif adv_action == 1:
                    self.state = np.random.choice([3, 4], p=[0.9, 0.1])
