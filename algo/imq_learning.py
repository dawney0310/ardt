...
def print_model_q_values(qsa_pr_model, qsa_adv_model, value_model, device, stage_name):
    """
    打印每个模型对不同状态和动作的Q值
    """
    print(f"\n=== {stage_name} - 模型Q值测试 ===")
    ...
    for i in [0, 1, 2, 3, 4, 5, 6]:
        obs = np.zeros(10)
        obs[i] = 1.0  # one-hot encoding
        states.append(obs)
    
    actions = [[1., 0.], [0., 1.]]  # 三个动作的one-hot编码
    adv_actions = [[1., 0.], [0., 1.]]  # 两个对手动作的one-hot编码
    
    print("\n--- QSA_PR_Model (玩家Q值) ---")
    for i, state in enumerate([0, 1, 2, 3, 4, 5, 6]):
        obs_tensor = torch.from_numpy(states[i]).float().to(device).unsqueeze(0).unsqueeze(0)  # [1, 1, 11]
        print(f"State {i}:")
        for j, action in enumerate(actions):
            action_tensor = torch.tensor([action]).float().to(device).unsqueeze(0)  # [1, 1, 3]
            with torch.no_grad():
                q_value = qsa_pr_model(obs_tensor, action_tensor)
                print(f"  Action {j}: Q = {q_value.item():.4f}")
    ...
    for i, state in enumerate([0, 1, 2, 3, 4, 5, 6]):
        ...
        for j, action in enumerate(actions):
            action_tensor = torch.tensor([action]).float().to(device).unsqueeze(0)  # [1, 1, 3]
            for k, adv_action in enumerate(adv_actions):
                adv_action_tensor = torch.tensor([adv_action]).float().to(device).unsqueeze(0)  # [1, 1, 2]
                with torch.no_grad():
                    q_value = qsa_adv_model(obs_tensor, action_tensor, adv_action_tensor)
                    print(f"  Action {j}, Adv {k}: Q = {q_value.item():.4f}")
    ...
    for i, state in enumerate([0, 1, 2, 3, 4, 5, 6]):
        ...
        with torch.no_grad():
            ...
            print(f"State {i}: V = {value.item():.4f}")
...
def imq_learning(
    ...
    # 在训练开始前打印初始Q值
    print_model_q_values(qsa_pr_model, qsa_adv_model, value_model, device, "训练开始前")
    ...
    # 在MSE训练开始前打印Q值
    print_model_q_values(qsa_pr_model, qsa_adv_model, value_model, device, "MSE训练开始前")
        ...
        for obs, acts, adv_acts, ret, seq_len in pbar:
            ...
            if epoch < mse_epochs:
                ...
                if epoch == mse_epochs - 1 and total_batches == len(dataloader):
                    ...
                    print_model_q_values(qsa_pr_model, qsa_adv_model, value_model, device, "MSE训练结束后")
               ...
               # Min step: adversary attempts to minimise at each node                        
                ...
                ret_tree_loss = (((value_pred[:, 1:].detach() + rewards - ret_adv_pred[:, :-1]) ** 2) * ~acts_mask[:, :-1].unsqueeze(-1)).mean()
                ret_leaf_loss = (
            ...
            pbar.set_description(
                ...
                f"Value Loss: {total_value_loss / total_batches:.4f}"
            )
        
        # 每个epoch结束时打印Q值
        print_model_q_values(qsa_pr_model, qsa_adv_model, value_model, device, f"Epoch {epoch} 结束")
